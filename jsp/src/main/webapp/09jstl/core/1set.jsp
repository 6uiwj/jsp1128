...
<!-- 서블릿은 페이지 전환이 있어야하기 때문에 request영역이 필요하지만 jsp는 화면 전환이 (웬만하면) 필요없기 때문에
scope 영역을 지정해주지 않아도(=page영역에 저장됨) 무방한 경우가 많음 -->
<c:set var="person" 
...
<h2>Map 컬렉션 이용하기</h2>
<%
	//맵: 다른타입의 여러가지 값을 저장하기가 용이, 이름으로 저정하고 이름으로 꺼내올 수 있음
	//ajax, petch를 이용할 경우 -> 데이터만 전달(화면X) -> 스크립트에서 데이터를 파싱하여 화면을 다시 그려주어야 함 
