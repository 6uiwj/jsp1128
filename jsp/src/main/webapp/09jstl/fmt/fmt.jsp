...
<br>type=percent 출력 : <fmt:formatNumber value="0.3" type="percent"></fmt:formatNumber>

<h2>패턴을 지정해서 숫자를 활용</h2>
0, # : 자릿수를 의미
<br>0 : 자리수를 지정했는데 해당 자리에 수가 없으면 0을 출력 ex)300 -> 00300 :자릿수를 맞추고 싶을 때(코드값을 출력) 
<br># : 자리수를 지정했는데 해당 자리에 수가 없으면 출력 하지 않음 ex)300 -> 300 :

<!-- 출력 형식을 지정하는 것은 매우 중요한 일이지만
...
0,00.0 :	<fmt:formatNumber value="${num1 }" pattern="0,00.0"/><br>
