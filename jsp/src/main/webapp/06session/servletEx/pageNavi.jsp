...
<br>${pageDto.prev? "":"disabled" }

<!-- 페이지 블럭을 출력하는 방법
	1. 요청 컨트롤러에서 pageDto를 requeqt영역에 저장해주세요
	2. pageNavi.jsp를 include해주세요!! - 리스트 페이지의 페이지블럭을 출력하고 싶은 위치에 
	3. serchForm을 생성합니다. pageNo 요소가 있어야 해요-->
<!-- 페이지네이션 -->
<nav aria-label="...">
  <ul class="pagination">
  <!--앞으로 가기 버튼 시작 / disabled : 비활성화 -->
    <li class='page-item ${pageDto.prev? "":"disabled"}'>
      ...
      <!-- href="/boardList?pageNo= -->
    </li>
    <!--  앞으로가기 버튼 끝 -->
<c:forEach var="i" begin="${pageDto.startNo }" 
		end="${pageDto.endNo }" >
    	<li class="page-item ">
    		<!-- href="..." 링크로 이동할 경우, 검색어가 유지되지 않습니다. -->
    		<!-- 검색어를 유지하기 위해 searchForm을 전송하는 goPage 함수를 생성 했습니다. 
    		링크를 함수호출로 변경 onClick 이벤트가 발생하면 goPage() 함수를 호출합니다.
