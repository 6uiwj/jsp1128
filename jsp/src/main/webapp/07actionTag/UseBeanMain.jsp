...
<%-- 
 자바빈이란?
 	요청발생시 화면으로부터 입력받은 데이터를 하나의 객체(DTO, VO)에 담아 편리하게 사용할 수 있습니다.
 ...
 자바빈으로 생성될수 있는 객체(DTO/VO)
	...
	- 기본 생성자가 존재해야 한다.
	- 멤버변수의 접근제어자는 private로 선언되어야 한다.(캡슐화를 위해)
	- 멤버변수에 접근 가능한 getter 와 setter 메서드가 존재해야 한다.
	- getter 와 setter는 접근자가 public으로 선언되어야 한다. 
	
<jsp:useBean> : 자바빈 생성
<jsp:setProperty> : 자바빈 속성을 저장합니다.(setter 호출)
<jsp:getProperty> : 자바빈 속성을 추출합니다.(getter 호출)
 --%>
...
<jsp:useBean id="boardDto" class="com.momo.dto.BoardDto"
